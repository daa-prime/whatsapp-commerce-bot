...
A customer can complete a full shop → cart → checkout conversation, receive a real Razorpay payment link, and — once they pay — get a WhatsApp confirmation with the order automatically marked `paid`, all race-safe under concurrent checkouts and duplicate webhook deliveries, in either English or Hindi throughout. A merchant can now separately log into their own portal to see that activity, manage orders/products, and self-serve their catalog/payment settings. Invoicing (Phase 4) is the only remaining piece of the core commerce loop; merchant self-serve signup (removing the admin-set-password step) is the natural next portal iteration once a real tenant has used it.

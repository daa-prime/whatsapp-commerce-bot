...
**Status (last updated after real Razorpay payment integration landed):**
...
- **Phase 6 (edge cases) — the stock/race-condition and payment-idempotency subsets are done; the rest is still a placeholder.** Hardened so far:
  ...
  - Still not addressed: `reminders/scheduler.py::send_abandoned_cart_nudges()` remains a no-op stub; duplicate *order-message* webhooks are unaddressed (Phase 2 isn't Meta's native flow yet, so there's no such webhook to duplicate).
- **Phase 7 (onboarding wizard: catalog/payment-gateway steps) — done for what's currently needed.** The create form and the `/admin/tenant/{id}/catalog-payment` edit step collect `meta_catalog_id`, `payment_gateway_provider`, and all three Razorpay credentials (`payment_gateway_key_id`/`payment_gateway_api_key_ref`/`payment_gateway_webhook_secret`) as optional fields; blank fields on the edit step never overwrite an existing value. Still missing: any UI for adding *products* (still manual via `db.create_product`).
- **Phase 8 (multi-tenant support beyond the pilot) — not applicable yet.**

A customer can complete a full shop → cart → checkout conversation, receive a real Razorpay payment link, and — once they pay — get a WhatsApp confirmation with the order automatically marked `paid`, all race-safe under concurrent checkouts and duplicate webhook deliveries. Invoicing (Phase 4) is the only remaining piece of the core commerce loop.

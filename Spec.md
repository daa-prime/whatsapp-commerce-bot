...
- **Phase 1 (catalog setup) — done, verified live.** Feed-based sync (`catalog/feed.py::build_feed_csv`, served at the new public `GET /catalog/{tenant_id}.csv`) rather than the Catalog Batch API — the Batch API needs a `catalog_management` Graph API permission gated behind Meta App Review for a production app, an external non-code approval process; a feed needs no such permission, only manual one-time Commerce Manager steps (create/link a catalog, register the feed URL as a data source). New `products.catalog_retailer_id` column (deterministic `f"t{tenant_id}-p{product.id}"`, auto-populated by `db.create_product` — not sourced from `products.sku`, since a real client's Shopify export had a SKU on only 7 of 1173 rows) is the stable id Meta's catalog/order-webhook references a product by; `db.get_product_by_retailer_id()` resolves the reverse direction. Confirmed against Meta's real catalog (`1094269489778440`, tenant 1's production deployment): Commerce Manager ingested all 18 registered demo products with matching Content IDs, all in stock, no item-level issues — the placeholder `link` value (this WhatsApp-only storefront's landing page, since no per-product page exists) was accepted fine. One real deployment gotcha found and fixed along the way: `PUBLIC_BASE_URL` had been left pointing at a local dev ngrok tunnel on the production deployment, making the feed's `link` column point at a URL that only existed while a dev machine had it running — fixed by pointing it at the real Railway domain.
- **Phase 2 (order-received webhook handling) — done; outbound native browsing verified live and working end-to-end, inbound order webhook still unverified (no real order sent yet).** `core/whatsapp.py::parse_incoming_message` now has an `order` branch (catalog_id + product_items), and `core/main.py`'s webhook handler dispatches an `order`-type message straight to new `core/commerce_flow.py::handle_native_order()` — bypassing the tap-driven session state machine entirely, since a native catalog cart has no prior bot conversation turn to resume. It maps each `product_retailer_id` back to a local product, drops any that don't resolve (stale/rejected catalog item, logged not fatal), and reuses `db.checkout_cart()` + a new shared `_complete_checkout_from_cart()` helper (extracted from the tap-driven `_checkout`, so both paths generate the Razorpay payment link and confirmation identically). New `WhatsAppClient.send_product_list()` sends Meta's native multi-product message (up to 30 items/10 sections, real product cards with images — no 24-char title cap, no per-row-image limitation) in place of the list-message workaround, automatically, for any tenant with `meta_catalog_id` set (`core/commerce_flow.py::_send_product_list` branches on it); every tenant without one keeps getting exactly today's list-message flow, unchanged — the explicit graceful-degradation fallback. `send_product_list()`/`_send_native_product_list()` now return a bool rather than swallowing the outcome — a **runtime** send failure (e.g. `meta_catalog_id` is set but Meta hasn't finished approving the catalog's items yet) falls back to the list-message flow too, not just the "no catalog configured at all" static case; found via a real deployed tenant that had `meta_catalog_id` unset, which clarified the fallback was actually a static pre-condition check, not a try/then-fallback -- the runtime-failure gap was real and is now closed, confirmed with a test that simulates a rejected native send. **Confirmed against a real Meta rejection, then fixed:** the first live production send failed -- Meta's Graph API rejected `send_product_list()`'s payload because it was missing `interactive.header`, which turned out to be required (not optional, unlike `send_list`/`send_buttons`'s headers) specifically for `product_list` messages. Verified against Meta's own Cloud API reference once found (`developers.facebook.com/docs/whatsapp/cloud-api/reference/messages`): *"HeaderObject is Required for 'product_list' type. Cannot be set for 'product' type."* `send_product_list()`'s `header_text` param is now required (no default) rather than optional, specifically so this can't silently regress -- `_send_native_product_list()` passes `tenant.name` as the header. This also means the earlier "graceful degradation" fallback (above) was real and load-bearing on the very first live attempt: the failed native send fell back to the list-message flow exactly as designed, with no visible break to the customer, while the actual Meta rejection reason only showed up in the server log. After the header fix, a second live attempt still failed with a different Meta error (131009, "None of the products provided could be sent") even though Commerce Manager confirmed the catalog was correctly populated -- root cause turned out to be outside this codebase entirely (the WhatsApp Business Account's catalog connection/access-token permissions on Meta's side, plus the `PUBLIC_BASE_URL` deployment misconfiguration above). Once the tenant's Meta access token was regenerated and the catalog properly connected on Meta's side, a retest against the real deployed tenant succeeded -- native `product_list` messages now render correctly for real customers. The incoming `order` webhook shape is still unverified -- no real order has been sent through native browsing yet; confirm once one is, same "unverified until a live payload confirms it" caution this file already applies to Razorpay's webhook shape below.
...
- **Phase 6 (edge cases) — the stock/race-condition and payment-idempotency subsets are done; the rest is still a placeholder.** Hardened so far:
  ...
  - Still not addressed: duplicate *order-message* webhooks. Now that Phase 2's native `order` webhook handling exists, this is a real (not just theoretical) gap — `handle_native_order()` is only protected by `core/main.py`'s per-(tenant, phone) message lock (prevents two *concurrent* deliveries racing each other), not a persistent dedupe key; a genuinely redelivered order webhook arriving after the first one already finished processing would create a second order. The tap-driven flow avoids this via session-state consumption (`_checkout`'s docstring), which a native order has none of. Flagged, not fixed — would need e.g. storing Meta's webhook message id and checking it before processing.
  - `db/connection.py`'s single process-lifetime Postgres connection now reconnects once on `OperationalError`/`InterfaceError` instead of crashing every subsequent query -- found during manual testing against Neon, whose pooler silently closes an idle connection after a few minutes (exactly what happens between WhatsApp messages during low-traffic use, not just an idle dev server).
...
- **Marketing landing page + orange wizard reskin — done.** `GET /` now serves `templates/landing.html` (moved off the old JSON health check, which lives at `GET /health` now — `railway.toml`'s `healthcheckPath` updated to match), matching CareConnect's landing structure (brand header with tagline, headline/subheadline, three feature cards, two CTAs + a secondary "Merchant login →" link, a WhatsApp phone mockup showing the *actual* `core/commerce_flow.py` main menu, not an invented one) reskinned in a considered terracotta/cream palette instead of CareConnect's dark green. The onboarding wizard (all 9 steps) and its confirmation page get the same terracotta theme via a new `templates/_orange_theme.html` partial, included only on those pages (`{% block head %}{% include "_orange_theme.html" %}{% endblock %}`) — it overrides `base.html`'s `:root` color tokens for that page only, so the merchant portal (`templates/portal/*`) stays on its original pink/magenta on purpose; this was a visual/content pass, wizard logic/validation/routes untouched.
- **Product browsing: categories + images — done.** WhatsApp's product list message (`core/commerce_flow.py::_send_product_list`) now groups active products into one list-section per category (first-appearance order, blank category → "Other"), a native list-message feature, not a workaround — pagination math (`MAX_LIST_ROWS`-row cap + "See more") is unchanged, just applied to the category-grouped flat order so a category's products page together. WhatsApp list messages can't show per-row images (a hard platform limit) — instead, tapping into a product's detail view (`_send_product_detail`) now sends its `image_url` as the interactive button message's image header (`core/whatsapp.py::send_buttons` gained `header_image_url`), falling back to no header if the product has no image.
...
- **Language selection (English/Hindi) — done.** New `core/strings.py`: every fixed bot-facing string (main menu, browsing, cart/checkout, orders, the `_PLEASE_CHOOSE` fallback, payment confirmations, abandoned-cart nudges) is a lookup keyed by language code (`t(key, lang, **kwargs)`), not scattered inline if/else — merchant-configured content (business name, product names/descriptions) is never auto-translated, only the bot's own UI text. A brand-new conversation sees a bilingual button prompt ("English" / "हिन्दी") before the main menu; the choice lives in `context["language"]`, the same session-context bag as everything else, and rides along for free through every handler's existing `{**context, ...}` merges. A *reset* (RESET_KEYWORDS — now including Hindi equivalents "नमस्ते"/"मेनू"/"शुरू"/"हाय" — a stale main-menu tap from another state, or any unrecognized-state fallback) preserves the already-chosen language rather than re-prompting; only a conversation that never picked one, or one whose session fully expired (30-min timeout discards context entirely), sees the prompt again. All three "return to idle" paths now route through one `_handle_idle`, which is what makes that preservation automatic rather than three copies of the same logic. New `orders.language` column snapshots the chosen language at checkout time (`db.checkout_cart`/`db.create_order`, same "snapshot at creation" reasoning as `unit_price_at_order_time`) — needed because `handle_payment_success`/`handle_payment_failure` (Razorpay webhook-triggered) and the abandoned-cart nudge job (external cron-triggered) both have no live session to read a language preference from by the time they fire, often well after the conversation itself expired. **Known limitation, not a bug:** Meta's native catalog flow (`handle_native_order`) has no conversational touchpoint at all before the final "Send order" webhook — browsing and cart-building happen entirely inside WhatsApp's own UI — so there's no way for a native-catalog customer to ever pick a language; those messages are always English.

- **Proactive fulfillment notification — done.** The portal's mark-fulfilled action (`portal/orders.py`'s `POST /portal/orders/{id}/fulfill`) now sends the customer a WhatsApp confirmation the moment a merchant marks their paid order fulfilled, via a new `core/commerce_flow.py::handle_order_fulfilled`, same shape as the existing `handle_payment_success`/`handle_payment_failure` webhook notifiers (language from `order.language`, no session involved). Only fires when `db.mark_order_fulfilled()`'s atomic `WHERE status = 'paid'` UPDATE reports it actually made the transition, so a duplicate/re-click never double-sends. Scoped to fulfillment only, on purpose — it's the only status transition the portal itself triggers today (payment success/failure already notify via the existing webhook path; there's no portal "mark failed/cancelled" action to hang a notification off yet). **Flagged, not built:** if a portal "cancel order" action gets added later, it should almost certainly get the same treatment — cancellation is exactly the kind of status change a customer wants proactively told about — but building that notification ahead of the action itself would be speculative.

A customer can complete a full shop → cart → checkout conversation, receive a real Razorpay payment link, and — once they pay — get a WhatsApp confirmation with the order automatically marked `paid`, all race-safe under concurrent checkouts and duplicate webhook deliveries, in either English or Hindi throughout, and gets a further WhatsApp message the moment the merchant ships it. A merchant can now separately log into their own portal to see that activity, manage orders/products, and self-serve their catalog/payment settings. Invoicing (Phase 4) is the only remaining piece of the core commerce loop; merchant self-serve signup (removing the admin-set-password step) is the natural next portal iteration once a real tenant has used it.

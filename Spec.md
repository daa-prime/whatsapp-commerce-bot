...
- **Phase 7 (onboarding wizard: catalog/payment-gateway steps) — done for what's currently needed.** The create form and the `/admin/tenant/{id}/catalog-payment` edit step collect `meta_catalog_id`, `payment_gateway_provider`, all three Razorpay credentials (`payment_gateway_key_id`/`payment_gateway_api_key_ref`/`payment_gateway_webhook_secret`), and `abandoned_cart_nudge_hours`, all optional; blank fields on the edit step never overwrite an existing value. Still missing: any UI for adding *products* (still manual via `db.create_product`).

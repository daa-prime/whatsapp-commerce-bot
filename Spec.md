...
**Status (last updated after hardening the shop/checkout flow's stock and race-condition handling):**
...
- **Phase 3 (payment integration) — not started.** Checkout creates a real `orders` row (`pending_payment`) + `order_items` + an atomic stock decrement, then stops at a "Payment integration coming next" placeholder message. No Razorpay (or any) SDK, no payment-link generation, no gateway webhook route yet.
...
- **Phase 5 (order/inventory data model) — done.** `products`, `orders`, and `order_items` tables in `db/schema.sql` with CRUD in `db/repository.py`, written to by `commerce_flow.py`'s checkout step.
- **Phase 6 (edge cases) — the stock/race-condition subset is now done; the rest is still a placeholder.** Hardened this pass:
  - Stock is checked at add-to-cart (advisory) and enforced atomically at checkout (`db.checkout_cart()`'s conditional `UPDATE ... WHERE stock_quantity >= ?`, race-safe under real concurrent access — proven by a genuine two-thread/two-connection test, not just a sequential simulation, in `tests/test_products_orders.py`).
  - Stock is decremented in the same DB transaction as order/order_items creation (`db/connection.py` gained a `transaction()` context manager for this one call site; every other call site is unaffected).
  - Duplicate checkout (double-tap, webhook redelivery) can't create two orders: the cart is cleared before the DB write, composing with the existing per-`(tenant, phone)` message lock.
  - A product that goes out of stock or gets deactivated between add-to-cart and checkout is dropped from the order with a clear message, not silently ordered or crashed on.
  - Still not addressed: `reminders/scheduler.py::send_abandoned_cart_nudges()` remains a no-op stub; payment-webhook/order-confirmation races (Phase 3 doesn't exist yet) and duplicate *order-message* webhooks (Phase 2 isn't Meta's native flow yet) are unaddressed.
- **Phase 7 (onboarding wizard: catalog/payment-gateway steps) — partially done.** The create form (`/admin/onboard-tenant`) now also collects `meta_catalog_id`/`payment_gateway_provider`/`payment_gateway_api_key_ref` as optional fields, and a separate edit step (`/admin/tenant/{id}/catalog-payment`) lets an operator add or update them later without re-onboarding (blank fields there never overwrite an existing value — `db.update_tenant_catalog_and_payment`'s COALESCE semantics). Still missing: any UI for adding *products* (still manual via `db.create_product`), and no payment gateway *integration* logic reads these stored fields yet — that's Phase 3.
- **Phase 8 (multi-tenant support beyond the pilot) — not applicable yet.**

`core/main.py`'s message handler dispatches to `core/commerce_flow.py`. A customer can complete a full shop → cart → checkout conversation and get a real `pending_payment` order with correctly decremented stock, safe under concurrent checkouts for the same product and against duplicate checkout taps; nothing past that (payment, invoicing) exists yet.
